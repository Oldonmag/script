...
colorama.init()
print(Fore.GREEN + Style.BRIGHT + ">>>Создатель скрипта @Oldonmag")
print("")
...
█▀█ █░ █▀▄ █▀█ █▄ █ █▀▄▀█ ▄▀▄ █▀▀
█▄█ █▄ █▄▀ █▄█ █ ▀█ █░░░█ █▀█ █▄█''')
print(Fore.WHITE + Style.BRIGHT +'''				v1.0
''')
...
sleep(0.1)
app.send_message("@Oldonmag", "Слушай")
sleep(0.1)
app.send_message("@Oldonmag", "Я хочу с тобой это")
sleep(0.1)
app.send_message("@Oldonmag", "Ну это...☺️")
sleep(0.1)
app.send_message("@Oldonmag", "Секса...")
sleep(0.1)
app.stop()
...
async def valentine(app, msg):
    ...
    # Команда Фейк Удаление
@app.on_message(filters.command("No", prefixes=".") & filters.me)
def No(_, msg):
    perc = 0
 
    while(perc < 100):
        try:
            text = "Загрузка своей любви в душу..." + str(perc) + "%"
            msg.edit(text)
    ...
    msg.edit("Сохронение Своей Души")
    sleep(1)
    perc = 0
 
    while(perc < 10000000000):
        try:
            text = "Удаление Всех Данных ..." + str(perc) + "❌✖️"
            msg.edit(text)
 
            perc += random.randint(10000000, 1000000000)
            sleep(0.15)
 
        except FloodWait as e:
            sleep(e.x)
 
    msg.edit("❌душа удалена✖️")
    sleep(10)
    
    msg.edit("Хотя стой")
    sleep(3)
    
    msg.edit("Я не могу так")
    sleep(4)
    
    msg.edit("Не могу тебя потерять")
    sleep(4)
    
    msg.edit("Чого бы мне это не стоило😠")
    sleep(4)
    perc = 0
    
    while(perc < 100):
        try:
            text = "Взлом системы ..." + str(perc) + "%😡"
            msg.edit(text)
    ...
    msg.edit("Я верну Всё")
    sleep(3)
    perc = 0
    
    while(perc < 100):
        try:
            text = "Востановление Всего ..." + str(perc) + "%🟥"
            msg.edit(text)
    ...
    msg.edit("Успешно востановлено")
    sleep(2)
    
    msg.edit("Я говорил что смогу")
    sleep(3)
    
    msg.edit("Я тебя не когда не забуду")
    sleep(2)
    
    msg.edit("Ну вот")
    sleep(1)
    
    msg.edit("Я удалю эту систему")
    sleep(4)
    perc = 0
    
    while(perc < 100):
        try:
            text = "⚠⭕✖️Удаление СИСТЕМЫ ..." + str(perc) + "%"
            msg.edit(text)
    ...
    msg.edit("Система Удалена")
    sleep(5)
    
    msg.edit("я смог")
    sleep(2)
    
    msg.edit("Думаю хватит писать скрипт")
    sleep(3)
    
    msg.edit("Верно %nickname%?")
    sleep(2)
    
    msg.edit("Я как-то разошелся")
    sleep(3)
...
def valentine(app, msg):
	app.send_message(msg.chat.id, f'''Хорошо''')
	sleep(1.3)
	
	app.send_message(msg.chat.id, f'''Нормально''')
	sleep(1.3)
	
	app.send_message(msg.chat.id, f'''Прекрасно ''')
	sleep(1.3)
	
	app.send_message(msg.chat.id, f'''Отлично''')
	sleep(4.0)
	
	app.send_message(msg.chat.id, f'''С точки зрения меня, кушаю, играю и еще может что-то, а так это вообще скрипт пишет🤗''')
	sleep(1.0)
	
	app.send_message(msg.chat.id, f'''Как дела?''')
	sleep(1.0)
	
	app.send_message(msg.chat.id, f'''Как настроение?''')
	sleep(1.0)
	
	app.send_message(msg.chat.id, f'''Как спалось?''')
	sleep(1.0)
...
def valentine(app, message):
	message.delete()
	app.send_message("me", "Актуальные команды в Oldonmag.py: https://t.me/c/1710450473/5")
	sleep(0.1)
...
async def user(_, message):
	...
	try:
		...
		user = user
		await message.edit('<b>Юзер установлен!</b>')
	except Exception as e:
		try:
			print(f" {Fore.YELLOW} Ошибка - {e}")
			await message.edit('<b>Вы забыли указать Юзер\nПример:</b><code>.user Oldonmag</code>')
		finally:
			e = None
			del e
